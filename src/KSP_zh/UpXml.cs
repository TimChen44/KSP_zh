        ...
        public static void CreateXmlUP(string ksppath, string nVel, string oVel, Label label)
        {
            ...
            foreach (var item in nVel_en)
            {
                ...
                {//找到匹配项
                    ...
                    //就条目中寻找中文
                    if (oZh.Value != null && IsHasCHZN(oZh.Value) == true)
                    {//找到中文条目
                        zh_up.AppendLine(string.Format(temp, item.Key, oZh.Value));//加入新中文条目
                        continue;
                    }
                ...
                zh_up.AppendLine(string.Format(temp, item.Key, item.Value));
            }
